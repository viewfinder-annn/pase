...
from models.mel.feature_extractor import Mel_feat as Encoder
from models.vocoder.vocos.vocoder import VocosVocoder as Generator
...
def run(rank, config, args):
    ...
    loss = MultiScaleMelSpectrogramLoss(**config['loss']).to(args.device)
...
class Trainer:
    ...
    def _train_epoch(self, epoch):
        ...
        for step, (true_wav, info) in enumerate(self.train_bar, 1):
            ...
            # For generator
            feat = self.encoder(true_wav.squeeze(1))  # (B, D, T)

            esti_wav = self.generator(feat)
            
            esti_wav = align_length(esti_wav, true_wav)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-C', '--config', default='configs/cfg_train_melvocoder.yaml')
